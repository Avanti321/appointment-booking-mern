...
const AddDoctor = () => {
  ...
  // ✅ Fix 1 — Add back the missing useEffect for image preview
  useEffect(() => {
    if (!docImg) {
      setPreview(null)
      return
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      // ✅ Fix 2 — Correct header format
      const { data } = await axios.post(
        backendUrl + '/api/admin/add-doctor',
        formData,
        { headers: { aToken } }  // ← change to whatever your backend checks e.g. Authorization: `Bearer ${aToken}`
      )
      ...
      toast.error(error.message)  // ✅ Fix 3 — empty catch was silently hiding all errors
      console.log(error)
    }
  }
      ...
      <div className='bg-white px-8 py-8 border rounded w-full max-w-4xl max-h-[80vh] overflow-y-scroll'>
        ...
        <div className='flex flex-col lg:flex-row items-start gap-10 text-gray-600'>
          ...
          <div className='w-full lg:flex-1 flex flex-col gap-4'>
            ...
            <div className='flex flex-col gap-1'>
              <p>Education</p>
              <input className='border rounded px-3 py-2' type="text" placeholder="Education details" value={degree} onChange={(e) => setDegree(e.target.value)} required />
            </div>
            <div className='flex flex-col gap-1'>
              <p>Address</p>
        ...
        <button className='bg-primary px-10 py-3 mt-4 text-white rounded-full' type="submit">
          Add Doctor
        </button>
