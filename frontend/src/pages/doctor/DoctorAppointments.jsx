  ...
  return (
    <div className='w-full max-w-6xl m-5'>
      ...
      <div className='bg-white border rounded text-sm max-h-[80vh] min-h-[50vh] overflow-y-scroll'>
        <div className='max-sm:hidden grid grid-cols-[0.5fr_2fr_1fr_1fr_3fr_1fr_2fr_1fr_1fr] gap-1 py-3 px-6 border-b'>
          <p>#</p><p>Patient</p><p>Payment</p><p>Age</p><p>Date & Time</p><p>Fees</p><p>Actions</p><p>Status</p><p>Rating</p>
        </div>

        {appointments.reverse().map((item, index) => (
          <div key={index} className='flex flex-wrap justify-between max-sm:gap-5 max-sm:text-base sm:grid grid-cols-[0.5fr_2fr_1fr_1fr_3fr_1fr_2fr_1fr_1fr] gap-1 items-center text-gray-500 py-3 px-6 border-b hover:bg-gray-50'>
            ...
            <div className='flex items-center gap-2'>
              ...
              <div>
                ...
                {/* Appointment type badge under patient name */}
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${
                  item.appointmentType === 'offline'
                    ? 'bg-orange-100 text-orange-600'
                    : 'bg-green-100 text-green-700'
                }`}>
                  {item.appointmentType === 'offline' ? '🏥 OPD' : '📹 Video'}
                </span>
              </div>
            </div>
            ...
            <div className='flex flex-col gap-1'>
              {/* Video call button — only for online appointments */}
              {!item.cancelled && !item.isCompleted && item.appointmentType === 'online' && (
                <button
              ...
              {/* OPD label — only for offline appointments */}
              {!item.cancelled && !item.isCompleted && item.appointmentType === 'offline' && (
                <span className='text-xs px-2 py-1 bg-orange-50 text-orange-500 border border-orange-200 rounded text-center'>
                  🏥 In-Person Visit
                </span>
              )}
            ...
            <div className='text-xs'>
              {item.isCompleted && item.isReviewed
                ? <div>
                    <p className='text-yellow-400 tracking-tight'>{'★'.repeat(item.rating)}{'☆'.repeat(5 - item.rating)}</p>
                    {item.review && <p className='text-gray-400 italic mt-0.5 max-w-[100px] truncate' title={item.review}>"{item.review}"</p>}
                  </div>
